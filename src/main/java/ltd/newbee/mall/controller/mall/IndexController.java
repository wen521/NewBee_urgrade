...
public class IndexController {
    ...
    public String indexPage(HttpServletRequest request) {
        ...
        request.getSession().setAttribute("categories", categories);//分类数据
        request.getSession().setAttribute("carousels", carousels);//轮播图
        request.getSession().setAttribute("hotGoodses", hotGoodses);//热销商品
        request.getSession().setAttribute("newGoodses", newGoodses);//新品
        request.getSession().setAttribute("recommendGoodses", recommendGoodses);//推荐商品
        return "mall/index";
    }
}
